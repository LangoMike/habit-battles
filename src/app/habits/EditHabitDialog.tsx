...
export default function EditHabitDialog({ habit, userId }: { habit: HabitLite; userId: string }) {
  const [open, setOpen] = useState(false);
  const [name, setName] = useState(habit.name ?? '');
  const [targetPerWeek, setTargetPerWeek] = useState<string>(String(habit.target_per_week ?? 3));
